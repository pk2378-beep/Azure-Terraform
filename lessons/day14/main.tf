...
data "azurerm_virtual_network" "rg-virtual_shared" {
  name = "shared-network-vnet"
  resource_group_name = data.azurerm_resource_group.rg-shared.name
}

data "azurerm_subnet" "rg-subnet_shared" {
  name = "shared-primary-sn"
  resource_group_name = data.azurerm_resource_group.rg-shared.name
  virtual_network_name = data.azurerm_virtual_network.rg-virtual_shared.name
}
...
resource "azurerm_network_interface" "main" {
  ...
  ip_configuration {
    ...
    subnet_id                     = data.azurerm_subnet.rg-subnet_shared.id
